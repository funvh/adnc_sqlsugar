...
public abstract partial class AbstractApplicationDependencyRegistrar
{
        ...
        Action<CapOptions>? replaceMqAction = null)
    {
        ...
        {
            var capConfig = CapSection.Get<CapConfigOptions>();
            ...
            {
                if (capConfig.DbType == "SqlServer")
                {
                    var schema = "Cap";
                    option.UseSqlServer(config =>
                    {
                        config.ConnectionString = capConfig.ConnectionString;
            ...
            option.SucceedMessageExpiredAfter = 30 * 24 * 3600;
            //消费失败保存时间
            option.FailedMessageExpiredAfter = 60 * 24 * 3600;
            //默认值：1,消费者线程并行处理消息的线程数，当这个值大于1时，将不能保证消息执行的顺序。
            option.ConsumerThreadCount = 5;
            //Dashboard
